...
export function renderAnalytics({data: loadedData, error, loading, filters = {}} = {}) {
  ...
  return `<section class="page page-analytics">${pageHeader('数据看板','四平台表现与单条作品复盘',`<button class="btn btn-primary" data-import-metrics aria-label="＋ 导入数据">${icon('plus')}导入数据</button>`)}
    ...
    <section class="content-section"><div class="section-heading"><div><h2>四平台表现</h2><p>账号与作品口径不混加，缺失数值显示 —</p></div></div><div class="platform-grid">${(data?.platforms || []).map(p=>`<article class="platform-card"><div><span class="platform-logo">${platformMark(p.name)}</span><strong>${escapeHtml(p.name)}</strong></div><span class="status-badge status-muted">${escapeHtml(qualityLabel(p.dataQuality))}</span><p>播放 / 阅读</p><b>${formatMetric(p.views)}</b><small>点赞 ${formatMetric(p.likes)} · 评论 ${formatMetric(p.comments)} · 净增 ${formatMetric(p.netFollowers)}</small></article>`).join('')}</div></section>
